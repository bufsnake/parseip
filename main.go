...
// 192.168.113.159
func SingleIP(ip string) []string {
	var ret = make([]string,0)
	for _, val := range strings.Split(ip,".") {
		ips, err := strconv.Atoi(val)
		if err != nil {
			panic(err.Error() + " ip parse error")
		}
		if ips > 255 {
			panic(ip +" ip parse error")
		}
	}
	ret = append(ret, ip)
	return ret
...
// 192.168.113.159-255
func MultipleIP(ips string) []string {
	var ret = make([]string,0)
	...
	if err != nil {
		panic("ip parse error")
	}
	if start > end {
		panic(ips+" ip parse error")
	}
	if start < 0 {
		start = 0
	}
...
// 192.168.113.159-192.168.113.254
func MultipleIP2(ips string) []string {
	var ret = make([]string,0)
...
// 192.168.113.0/24
func MultipleIP3(ips string) []string {
	var ret = make([]string,0)
...
// 192.167.113.159-192.168.114.254
// 192.168.113.159-192.168.114.254
func MultipleIP4(ips string) []string {
	var ret = make([]string,0)
	start := strings.Split(strings.Split(ips,"-")[0],".")
	end := strings.Split(strings.Split(ips,"-")[1],".")
	var i = 0
